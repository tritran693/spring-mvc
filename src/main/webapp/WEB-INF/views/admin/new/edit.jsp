		...
		<div class="page-content">
			<div class="row">
				<div class="col-xs-12">
					...
					<!-- FROM CỦA SPRING, modelAtribute phải bind biến model mà mình truyền lên -->
					<!-- Phần path ở dưới để match với các thuộc tính trong model -->
					<form:form class="form-horizontal" role="form" id="formSubmit" modelAttribute="model">
						<div class="form-group">
							  <label for="categoryCode" class="col-sm-3 control-label no-padding-right">Thể loại:</label>
							  <div class="col-sm-9">
							  	 <!-- Với categoryCode của model thì ứng với categories nào thì nó sẽ tự chọn -->
							  	 <form:select path="categoryCode" id="categoryCode">
							  	 	<form:option value="" label="-- Chọn thể loại --"/>
							  	 	<form:options items="${categories}"/>  <!-- Truyền vào 1 map, bên dưới sẽ tự biết model đang ở categories nào, đỡ mắc công check rồi selected như bên làm thuần -->
							  	 </form:select>
							  </div>
						</div>
						<div class="form-group">
...
<script>
	$('#btnAddOrUpdateNew').click(function (e) {
	    e.preventDefault(); //Nếu ko khai báo thì sẽ submit vào cái link hiện tại đang đứng :))
	    var data = {};
	    var formData = $('#formSubmit').serializeArray(); //chuyển về một mảng chứa các giá trị [{name:"",value:""},{},..]
	...
	function addNew(data) {
		$.ajax({
            ...
            data: JSON.stringify(data), //Chuyển từ js object to json
            dataType: 'json',
            success: function (result) { //result là DTO trả về
